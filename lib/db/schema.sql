-- Freckled Hen Inventory Management Schema
-- Optimized for 3,000+ SKUs with variants
-- Supports Shopify sync and AI automation
...
CREATE TABLE IF NOT EXISTS shopify_variants (
  ...
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  synced_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_shopify_variants_sku (sku),
  INDEX idx_shopify_variants_barcode (barcode),
  INDEX idx_shopify_variants_product (product_id),
  INDEX idx_shopify_variants_inventory (inventory_quantity),
  INDEX idx_shopify_variants_synced (synced_at)
);
...
CREATE TABLE IF NOT EXISTS shopify_inventory_levels (
  ...
  INDEX idx_inventory_levels_variant (variant_id),
  INDEX idx_inventory_levels_location (location_id),
  INDEX idx_inventory_levels_synced (synced_at)
);
...
CREATE TABLE IF NOT EXISTS inventory_alerts (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_inventory_alerts_variant (variant_id),
...
-- AI processing queue for automation
CREATE TABLE IF NOT EXISTS ai_processing_queue (
  ...
  INDEX idx_ai_queue_priority (priority, scheduled_at),
  INDEX idx_ai_queue_variant (variant_id)
);
...
CREATE TABLE IF NOT EXISTS shopify_collections (
  ...
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  synced_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_collections_handle (handle),
...
CREATE TABLE IF NOT EXISTS shopify_metafields (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_metafields_owner (owner_id, owner_resource),
  INDEX idx_metafields_namespace_key (namespace, key)
);
